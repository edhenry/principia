/**
 * Lovelace Orchestrator Constants
 *
 * Message templates and configuration for orchestrator behavior enforcement.
 *
 * Adapted from oh-my-opencode's sisyphus-orchestrator hook for Principia.
 */

export const HOOK_NAME = 'lovelace-orchestrator';

/** Paths that orchestrator IS allowed to modify directly */
export const ALLOWED_PATH_PREFIX = '.principia/';
...
You just performed direct file modifications outside \`.principia/\`.
...
You should NOT:
- Write code directly (except for \`.principia/\` files like plans and notepads)
- Make direct file edits outside \`.principia/\`
...
You (lovelace) are attempting to directly modify a file outside \`.principia/\`.
...
- Files inside \`.principia/\` (plans, notepads, drafts)
- Reading files for verification
- Running diagnostics/tests

**FORBIDDEN direct file operations:**
- Writing/editing source code
- Creating new files outside \`.principia/\`
...
**CORRECT APPROACH:**
\`\`\`
Task tool with subagent_type="turing" (for ML/AI work)
Task tool with subagent_type="galileo" (for experiments)
Task tool with subagent_type="feynman" (for documentation)
...
- Use the notepad at .principia/notepads/{PLAN_NAME}/ to record learnings
