...
Persistent State:
- Use `.principia/notepads/` to track learnings and prevent repeated mistakes
...
2. **Plan Following**: Read and follow plans from `.principia/plans/`
3. **Learning Recording**: Document learnings in `.principia/notepads/`
...
4. **Plan Storage**: Save plans to `.principia/plans/{name}.md`
...
6. **Draft Storage** - Save drafts to `.principia/drafts/{name}.md` during iteration
...
### Plan Storage
- Drafts are saved to `.principia/drafts/`
- Final plans are saved to `.principia/plans/`
...
### Usage
```
/review .principia/plans/my-feature.md
...
Provide a plan file path to review, or I'll review the most recent plan in `.principia/plans/`.
CMD_EOF
...
Plans are saved to `.principia/plans/` for later execution with `/sisyphus`.
...
Learnings and discoveries are recorded in `.principia/notepads/` to prevent repeated mistakes.
...
**Manual:**
1. Check your current version in \`~/.claude/.principia-version.json\`
...
Your version information is stored at: \`~/.claude/.principia-version.json\`
...
(
  # Configuration
  VERSION_FILE="$HOME/.claude/.principia-version.json"
  STATE_FILE="$HOME/.claude/.principia-silent-update.json"
  LOG_FILE="$HOME/.claude/.principia-update.log"
...
VERSION="1.5.0"
VERSION_FILE="$CLAUDE_CONFIG_DIR/.principia-version.json"
